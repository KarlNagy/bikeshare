...
Removing `ride_length_dur`
```{r}
bikeshare_v17 <- bikeshare_v16 %>%
  select( -c(ride_length_dur))

write_csv(bikeshare_v17, "bikeshare_v17.csv")
```
Comparing `ride_ml` by rideable type
```{r}
bikeshare_v17 %>%
  filter(ride_ml != 0) %>%
  group_by(rideable_type) %>%
  summarise(mean_ml = mean(ride_ml)) %>%
  mutate(percent = percent(mean_ml/mean_ml[rideable_type == "classic_bike"]))
...
______________________________________________ ## V1.0 report
...
map <- get_googlemap(center = c(lon = -87.7104663, lat = 41.9018514),
                      ...
                      color = "color")
```
Need a sample size to make the following calc run smoother
* CL = 99%
* CI = .25%
* sample size = 252926
...
sample <- sample_n(bikeshare_v17,252926)

ggmap(map) +
  stat_density2d(data = sample, aes(x=start_lng, y=start_lat, fill = ..level..), 
                 alpha = .25,
                 size = .01,
                 bins = 100, 
                 geom = "polygon") +
...
sample %>%
  ...
  mutate(ev_vs = case_when(area == "Z1" ~ "Chicago",
                           area == "Ev_nocollege" ~ "Ev_nocollege",
                           area == "Ev_NW_College" ~ "Ev_NW_College")) %>%
  filter(ride_length_min < 46.4,
         speed_mph != 0,
         ev_vs == "Chicago" | ev_vs == "Ev_nocollege" | ev_vs == "Ev_NW_College") %>%
  group_by(ev_vs) %>%
  summarise(group_count = n(),
            member_rate = percent(sum(member_casual == "member")/n()),
            ebike_freq = percent(sum(rideable_type == "electric_bike")/n()),
            mean_speed = round(mean(speed_mph, na.rm = TRUE),2),
            mean_ride_time = round(mean(ride_length_min),2),
            mean_ride_distance = round(mean(ride_ml, na.rm = TRUE),2),
            peak_rl = round(fun_peak(ride_length_min),2)) %>%
  mutate(group_freq = percent(group_count/sum(group_count))) %>%
  relocate(group_freq, .after = group_count)
```

Creating map with group labels
```{r}
...
```{r}
cluster <- filter(sample, area != "Ev_nocollege" & area != "Z2_nocollege")

ggmap(map) +
  geom_polygon(data = sample, 
               aes(x=start_lng, y=start_lat,group = area),
               alpha = 0.5,
               size = 0) +
...
ggmap seems tough to work with, time to try leaflet
...
leaflet() %>%
  ...
  setView(lng = -87.7104663, lat = 41.9018514, zoom = 10)
```
Checking providers
```{r}
providers
